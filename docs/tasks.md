...
- [ ] 📦 ライブラリの導入
  - [ ] `leaflet` と `react-leaflet`、型定義 `@types/leaflet` のインストール
- [ ] 🧠 AI ロジックの強化
  - [ ] `PlanData` (Zod Schema) に `lat`, `lng` (緯度経度) フィールドを追加
  - [ ] プロンプトを更新して、各スポットとホテルの座標を出力させる
- [ ] 🗺️ 地図コンポーネントの実装
  - [ ] `MapSection.tsx` を作成 (地図表示、ピン立て)
  - [ ] Next.js で Leaflet を使うための動的インポート対応 (SSR回避)
- [ ] 📍 画面への統合
  - [ ] プラン結果画面に地図を表示し、おすすめスポットにピンを立てる

## 現在のステータス: 🚀 Phase 7 始動！地図を広げて冒険へ！🗺️
