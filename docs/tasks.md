...
- [ ] 🧹 装飾の削除
  - [ ] 背景の泡アニメーション (`BackgroundBubbles`) を削除
  - [ ] 背景のグラデーションを削除し、クリーンな白/グレーベースに変更
- [ ] 🎨 カラー & スタイルの再定義
  - [ ] グラデーション文字・ボタンを廃止し、ソリッドカラーに変更
  - [ ] `glass-panel` を廃止し、`simple-card`（白背景+極薄ボーダー+淡い影）へ移行
  - [ ] テキスト色をニュートラルなダークグレー (`slate-900`) に統一し、可読性を最大化
- [ ] 🧩 コンポーネントの修正
  - [ ] Header/Footer: シンプルな白背景に変更
  - [ ] Form/Result: フラットでモダンなデザインに統一