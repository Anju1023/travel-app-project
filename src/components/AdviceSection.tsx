      ...
      <div className="glass-card p-6 md:p-8 rounded-4xl shadow-sm">
        ...
        <div className="space-y-4">
          {packingList.map((item, i) => (
            <div key={i} className="flex items-start gap-4 p-4 bg-white/40 rounded-2xl hover:bg-white/60 transition-all group border border-transparent hover:border-sky-100">
        ...
        <div className="space-y-5">
          {advice.map((item, i) => (
            <div key={i} className="relative pl-6 group">
              {/* 装飾用のライン */}
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-amber-200 to-orange-100 rounded-full group-hover:from-amber-400 group-hover:to-orange-300 transition-all duration-300" />
