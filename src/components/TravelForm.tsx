...
import { MapPin, Calendar, Wallet, Users, Heart, Sparkles, Luggage } from 'lucide-react';
 ...
 */
export default function TravelForm({ onSubmit }: { onSubmit: (data: TravelFormData) => Promise<void> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full space-y-8 bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-secondary/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <select name="duration" className="w-full p-4 bg-accent/30 rounded-2xl border-2 border-transparent focus:border-primary/50 focus:bg-white transition-all outline-none appearance-none cursor-pointer">
              <option>日帰り</option>
              <option>1泊2日</option>
              <option>2泊3日</option>
              <option>3泊4日</option>
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <select name="budget" className="w-full p-4 bg-accent/30 rounded-2xl border-2 border-transparent focus:border-primary/50 focus:bg-white transition-all outline-none appearance-none cursor-pointer">
        ...
        <div className="flex flex-wrap gap-3">
          {['一人旅', '友達', 'カップル/夫婦', '家族'].map((item) => (
            <label key={item} className="cursor-pointer group">
              <input type="radio" name="companions" value={item} className="hidden peer" defaultChecked={item === '友達'} />
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-3">
          {['のんびり', 'アクティブ', 'グルメ', '観光名所', '穴場スポット', '映え'].map((item) => (
            <label key={item} className="cursor-pointer">
              <input type="checkbox" name="style" value={item} className="hidden peer" />
      ...
      <div className="space-y-3">
        ...
        <textarea 
          ...
          className="w-full p-4 bg-accent/30 rounded-2xl border-2 border-transparent focus:border-primary/50 focus:bg-white transition-all outline-none min-h-[100px] resize-none"
        />
      </div>
      ...
      */}
      <button 
        type="submit" 
        disabled={loading}
        className="w-full py-5 rounded-2xl bg-gradient-to-r from-primary to-cyan-400 text-white font-bold text-xl shadow-lg shadow-primary/30 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
            <Luggage className="animate-bounce" /> {/* カバンが弾むアニメーション！ */}
            最高のプランを考え中...
          </>
        ) : (
          <>
