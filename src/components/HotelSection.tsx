		...
		<div className="glass-card p-6 md:p-8 rounded-4xl shadow-sm">
			...
			<div className="grid grid-cols-1 gap-6">
				{hotels.map((hotel, i) => (
					<div
						key={i}
						className="p-5 md:p-6 bg-white/40 rounded-3xl flex flex-col md:flex-row gap-6 items-start md:items-center justify-between hover:bg-white/80 transition-all duration-300 border border-transparent hover:border-indigo-100 hover:shadow-md group"
					>
						<div className="space-y-4 flex-1">
							...
							<div className="flex flex-wrap gap-2">
								{hotel.features.map((f) => (
									<span
										key={f}
										className="text-[10px] md:text-xs px-3 py-1 bg-white/60 text-indigo-500 rounded-full border border-indigo-50 font-bold tracking-wider uppercase shadow-sm"
									>
										{f}
									</span>
								))}
						...
						<div className="text-right w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-slate-100 flex md:flex-col items-center md:items-end justify-between md:justify-center gap-1">
							...
							<div className="font-black text-2xl text-transparent bg-clip-text bg-linear-to-r from-indigo-500 to-sky-400 tracking-tight">
								{hotel.price}
							</div>
						</div>
					</div>
